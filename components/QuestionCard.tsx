...
export default function QuestionCard({
  ...
  return (
    <div className="min-w-0 rounded-xl bg-white p-4 shadow-card sm:p-7">
      <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-gold-600">
        {t(language, "questionOf", { current: index + 1, total })}
      </p>
      <h2 className="text-wrap-safe mb-5 text-base font-semibold leading-relaxed text-charcoal sm:text-xl">
        ...
        <div className="space-y-3">
          {OPTION_KEYS.map((key) => {
            ...
            return (
              <label
                key={key}
                className={`flex min-w-0 cursor-pointer items-start gap-3 rounded-xl border-2 px-3 py-3 transition-colors sm:px-4 sm:py-3.5 ${
                  isSelected
              ...
              >
                ...
                />
                <span className="text-wrap-safe min-w-0 text-sm leading-relaxed text-gray-800 sm:text-base">
